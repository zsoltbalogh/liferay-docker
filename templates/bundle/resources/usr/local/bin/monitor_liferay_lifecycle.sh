...
function main {
	...
	do
		...
		curl_content=$(curl --connect-timeout "${LIFERAY_CONTAINER_STATUS_REQUEST_TIMEOUT}" --fail --max-time "${curl_max_time}" --show-error --silent --header "Host: ${LIFERAY_CONTAINER_STATUS_REQUEST_URL}" --url "localhost:8080" 2>/dev/null)
