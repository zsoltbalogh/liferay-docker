...
function get_token {
	local token=$(curl --fail --location --request POST --show-error --silent "http://${ORCA_VAULT_ADDRESSES}/v1/auth/userpass-${1}/login/${1}" --data '{"password": "'${ORCA_VAULT_SERVICE_PASSWORD}'"}')
...
function load_secrets {
	...
	do
		...
		local password=$(curl --fail --header "X-Vault-Token: ${token}" --location --request GET --show-error --silent "http://${ORCA_VAULT_ADDRESSES}/v1/secret/data/${secret}")
...
function wait_for_vault {
	...
	do
		...
		do
			if ( curl --fail --location --max-time 3 --show-error --silent "http://${ORCA_VAULT_ADDRESSES}/v1/sys/health" | grep "\"sealed\":false" &>/dev/null)
			then
