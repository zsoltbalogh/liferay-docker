...
function main {
	...
	then
		...
		curl --fail --location --header "${LIFERAY_DOCKER_LICENSE_API_HEADER}" --show-error --silent "${LIFERAY_DOCKER_LICENSE_API_URL}?licenseLifetime=$((1000 *60 * 60 * 24 * 90))&startDate=${license_start_date}&owner=docker%40liferay.com" > "${license_dir}/deploy/${license_file_name}.json"
