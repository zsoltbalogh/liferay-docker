...
function build_bundle_image {
	...
	if [ -z "${bundle_url}" ]
	then
		bundle_url="releases-cdn.liferay.com/dxp/${version}/"$(curl --fail --show-error --silent --location "https://releases-cdn.liferay.com/dxp/${version}/.lfrrelease-tomcat-bundle")
	fi
...
function get_latest_available_zulu_version {
	local version=$(curl --fail -H 'accept: */*' --location --show-error --silent "https://api.azul.com/zulu/download/community/v1.0/bundles/latest/?arch=${2}&bundle_type=jdk&ext=deb&hw_bitness=64&javafx=false&java_version=${1}&os=linux" | jq -r '.zulu_version | join(".")' | cut -f1,2,3 -d'.')

	echo "${version}"
}

function get_latest_docker_hub_version {
	local token=$(curl --fail --location --show-error --silent "https://auth.docker.io/token?scope=repository:liferay/${1}:pull&service=registry.docker.io" | jq -r '.token')

	local version=$(curl --fail --location --show-error --silent -H "Authorization: Bearer $token" "https://registry-1.docker.io/v2/liferay/${1}/manifests/latest" | grep -o '\\"org.label-schema.version\\":\\"[0-9]\.[0-9]\.[0-9]*\\"' | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
...
function get_latest_docker_hub_zabbix_server_version {
	...
	local token=$(curl --fail --location --show-error --silent "https://auth.docker.io/token?scope=repository:${image_tag}:pull&service=registry.docker.io" | jq -r '.token')
	...
	local version=$(curl --fail --location --show-error --silent -H "Authorization: Bearer $token" "https://registry-1.docker.io/v2/${image_tag}/manifests/${tag}" | grep -o "\\\\\"${label_name}\\\\\":\\\\\"[0-9]*\.[0-9]*\.[0-9]*\\\\\"" | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
...
function get_latest_docker_hub_zulu_version {
	...
	local version=$(curl --fail --location --show-error --silent -H "Authorization: Bearer $token" "https://registry-1.docker.io/v2/liferay/${1}/manifests/latest" | grep -o "\\\\\"org.label-schema.zulu${2}_${3}_version\\\\\":\\\\\"[0-9]*\.[0-9]*\.[0-9]*\\\\\"" | head -1 | sed 's/\\"//g' | sed 's:.*\:::')
