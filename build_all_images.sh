...
function build_base_image {
	...
	build_base_image_output=$(time ./build_base_image.sh ${BUILD_ALL_IMAGES_PUSH} 2>&1)
	build_base_image_ret=$?

	echo -e "${build_base_image_output}" | tee "${LOGS_DIR}"/base.log

	if [ $build_base_image_ret -gt 0 ]
	then
