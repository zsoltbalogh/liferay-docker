...
function upload_bom_file {
	...
	if [ "${nexus_repository_name}" == "xanadu" ]
	then
		local file_prefix="${file_name%.*}"
		local file_suffix="${file_name##*.}"

		local file_url="${nexus_repository_url}/${nexus_repository_name}/content/com/liferay/portal/${component_name}/${_DXP_VERSION}-${_BUILD_TIMESTAMP}/${file_prefix}-${_BUILD_TIMESTAMP}-${file_suffix}"
	else
